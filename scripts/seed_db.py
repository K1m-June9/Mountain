...
def create_sample_comments(cursor):
    ...
    # 샘플 댓글 생성
    sample_comments = [
        ('좋은 공지사항입니다.', 2, post_ids[0], None),
        ('감사합니다!', 1, post_ids[0], 1),
        ('환영합니다!', 3, post_ids[1], None),
        ('저도 같은 질문이 있었어요.', 5, post_ids[2], None),
        ('답변드립니다. 이렇게 사용하시면 됩니다.', 1, post_ids[2], 4),
        ('유용한 정보 감사합니다.', 4, post_ids[3], None),
        ('건의사항 검토하겠습니다.', 1, post_ids[4], None)
    ]
...
def create_sample_notices(cursor):
    ...
    # 샘플 공지사항 생성
    sample_notices = [
        ('사이트 오픈 안내', '안녕하세요. Mountain 커뮤니티가 오픈했습니다.', admin_id, True),
