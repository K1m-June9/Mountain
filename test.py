...
def create_sample_reactions(db: Session, users, posts, comments):
    ...
    # 게시물에 대한 반응
    post_reactions = [
        Reaction(user_id=users[1].id, post_id=posts[0].id, type='like'),
        Reaction(user_id=users[2].id, post_id=posts[0].id, type='like'),
        Reaction(user_id=users[3].id, post_id=posts[0].id, type='dislike'),
        Reaction(user_id=users[0].id, post_id=posts[1].id, type='like'),
        Reaction(user_id=users[4].id, post_id=posts[2].id, type='like')
    ]
    
    # 댓글에 대한 반응
    comment_reactions = [
        Reaction(user_id=users[0].id, comment_id=comments[0].id, type='like'),
        Reaction(user_id=users[2].id, comment_id=comments[1].id, type='like'),
        Reaction(user_id=users[3].id, comment_id=comments[2].id, type='dislike')
    ]
    
    for reaction in post_reactions + comment_reactions:
        db.add(reaction)
    
    db.commit()
    print(f"반응 생성: {len(post_reactions)} 게시물 반응, {len(comment_reactions)} 댓글 반응")
